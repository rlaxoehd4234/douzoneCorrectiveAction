...
import com.douzone.comet.service.eh.abc.dao.Abc00100DropDAO;
...
public class Abc00100Service extends DzCometService {
	...
	@Autowired
	Abc00100DropDAO abc00100DropDAO;
...
//						if(ehsstd01200DAO.countEhsstd01200MstModel(param) > 0){
//							list.add(ehsstd01200DAO.dupCheckEhsstd01200MstModel(param));
//							
//							mResult.setSuccess(false);
//							mResult.setParams(list);
//							mResult.setMsg((String)param.get("P_TMPLT_CD")+" 은 이미 존재하는 데이터입니다.");
...
//						}
	...
	@DzApi(url="/abc00100_drop_list", desc="시정조치요구등록드롭다운-조회", httpMethod=DzRequestMethod.GET)
	public List<Abc00100DropModel> abc00100_drop_list(
			@DzParam(key="companyCode", desc="회사코드", paramType = DzParamType.QueryString) String companyCode,
			@DzParam(key="MCLS_CD", desc="관리항목코드", paramType = DzParamType.QueryString) String MCLS_CD
	) throws Exception {
		List<Abc00100DropModel> abc00100DropList =  new ArrayList<Abc00100DropModel>();
		try {
	 		HashMap<String, Object> param = new HashMap<String, Object>();
	 		...
	 		abc00100DropList = abc00100DropDAO.selectAbc00100DropModelList(param);
	 	} catch(Exception e) {
	        throw new DzApplicationRuntimeException(e);
	    }
	    
	    return abc00100DropList;
 	}
}
